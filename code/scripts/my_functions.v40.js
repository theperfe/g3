...
//  console.log(JSON.stringify(final_matches));
  var negative_edges = graph1.links.filter(function(element){return element.marker_id[0] == '!';});
...
//      lhs_source_node_key = negative_edges[i].source.key; 
	...
	if(match_source_node.length != 0 && match_target_node.length != 0){
	  ...
	  console.log(JSON.stringify(mg_source_node));
	  console.log(JSON.stringify(mg_target_node));
	  ...
	  if(j<temp_successors.length) final_matches.splice(final_matches.indexOf(element),1);
	}
      });
    }
  }
