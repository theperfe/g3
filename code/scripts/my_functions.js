...
function remove_duplicate_matches_specific_to_NVFO(graph1, graph2, matches_list){  //this is a function specific to NVFO grammar; should disable this while making this code public or modify it accordingly.
  var links = [], lhs_source_node_label = null, lhs_target_node_label = null;
  links = graph1.links.filter(function(element){return element.definition.slice(0,5)=="new::";});
  if(links.length != 0 && (links[0].label == 'con' || links[0].label == 'dis')){
    if(typeof links[0].source == "object") lhs_source_node_label = links[0].source.label;
    else lhs_source_node_label = graph1.nodes[links[0].source].label;
    if(typeof links[0].target == "object") lhs_target_node_label = links[0].target.label;
    else lhs_target_node_label = graph1.nodes[links[0].target].label;
    if(lhs_source_node_label != lhs_target_node_label) return matches_list;
    else{	//if the node labels of both the nodes are same, then there are definitely duplicates. My rules are such.
      var i = matches_list.length - 1;
      while(i >= 0){	//because duplicate matches come consecutively
    ...
    var nodes = [];
    nodes = graph1.nodes.filter(function(element){return element.definition.slice(0,5)=='new::';});
    if(nodes.length != 0 && (nodes[0].label == 'q' || nodes[0].label == 's')){
      var j = matches_list.length - 1;
      while(j >= 0){	//because duplicate matches come consecutively
        matches_list.splice(j,1);
	j = j-2;
      }
    }
  }
...
function match_final(graph1, graph2){
  console.log("entered match final");
  console.log(graph1.name);
  console.log(graph2.name);
  var matches_tree = null, matches_list = [[]];
  matches_tree = match(graph1, graph2);
  console.log(JSON.stringify(matches_tree));
  if(graph1.nac_count == undefined && graph1.rule_name == undefined) {
//    console.log(graph1.name);
 //   console.log(graph2.name);
//    console.log(JSON.stringify(matches_tree));
  }
  matches_list = extract_all_matches(matches_tree);
  console.log("extracted all matches");
  if(graph1.nac_count == undefined) return matches_list;  //if lhs is a main graph or nac, just return the matches list as it is
//  return filter_matches_with_negative_edges(graph1, graph2, matches_list); //not necessary right now
  else return remove_duplicate_matches_specific_to_NVFO(graph1, graph2, matches_list);  //if lhs is a generative or interpretation rule, remove duplicate matches
}
...
//    lhs_temp_arr = graph1.nodes.filter(function(element){return (element.label == graph1.node_types[i] && (element.key in graph1.representative_nodes_list));});
    lhs_temp_arr = graph1.nodes.filter(function(element){return (element.label == graph1.node_types[i] && (graph1.representative_nodes_list.indexOf(element.key) != -1));});
...
//  console.log(JSON.stringify(mega_perms));
...
//     current_node.rule_index = representative_nodes_list_typewise[p].key;	//rule_index refers to the index of the node in the rule graph with which this node is matching.
     current_node.rule_index = graph1.nodes.indexOf(graph1.nodes.filter(function(element){return element.key == representative_nodes_list_typewise[p].key;})[0]);	
     current_node_obj = {'type':'node', 'name':current_node, 'child':[]};
     for(var q=0; q<graph1.link_types.length; q++){	//for a given link type
    if(graph1.nac_count == undefined && graph1.rule_name == undefined && graph1.name == "graph8" && graph2.name == "graph7") {
      console.log(representative_nodes_list_typewise[p].key);
      console.log(mega_perms[i][p].key);
      console.log(JSON.stringify(extract_relevant_data_from_list_of_nodes(predecessors(graph1, representative_nodes_list_typewise[p], graph1.link_types[q]))));
      console.log(JSON.stringify(extract_relevant_data_from_list_of_nodes(predecessors(graph2, mega_perms[i][p], graph1.link_types[q]))));
      console.log(JSON.stringify(extract_relevant_data_from_list_of_nodes(representative_nodes_list_typewise)));
      console.log(JSON.stringify(extract_relevant_data_from_list_of_nodes(mega_perms[i])));
    }
    ...
    if(graph1.nac_count == undefined && graph1.rule_name == undefined && graph1.name == "graph8" && graph2.name == "graph7") {
      console.log("came back from result1 of lhs key of " + representative_nodes_list_typewise[p].key + " and mg key of " + mega_perms[i][p].key + " for link type " + graph1.link_types[q]);
      console.log(representative_nodes_list_typewise[p].key);
      console.log(mega_perms[i][p].key);
      console.log(JSON.stringify(extract_relevant_data_from_list_of_nodes(successors(graph1, representative_nodes_list_typewise[p], graph1.link_types[q]))));
      console.log(JSON.stringify(extract_relevant_data_from_list_of_nodes(successors(graph2, mega_perms[i][p], graph1.link_types[q]))));
      console.log(JSON.stringify(extract_relevant_data_from_list_of_nodes(representative_nodes_list_typewise)));
      console.log(JSON.stringify(extract_relevant_data_from_list_of_nodes(mega_perms[i])));
    }
    ...
    if(graph1.nac_count == undefined && graph1.rule_name == undefined && graph1.name == "graph8" && graph2.name == "graph7") {
      console.log("came back from result2 of lhs key of " + representative_nodes_list_typewise[p].key + " and mg key of " + mega_perms[i][p].key + " for link type " + graph1.link_types[q]);
      console.log(JSON.stringify(extract_relevant_data_from_list_of_nodes(representative_nodes_list_typewise)));
      console.log(JSON.stringify(extract_relevant_data_from_list_of_nodes(mega_perms[i])));
    }
       if(result1 && result2){
...
//  console.log("entered sub match");
  var matchable_nodes = [], rlpslt = [], rmpslt = [];
  ...
  var lstate = 0, mstate = 0; //state variables to find out if 
...
//  rmpsl = mpsl.filter(function(element){return mvp.indexOf(element) != -1;})    //repeating nodes in mpsl
  var rlpsl = [] 
  lpsl.forEach(function(el){
    lstate = 0;
    lvp.forEach(function(el2){if(el.key == el2.key) lstate = 1;});
    if(lstate == 1) rlpsl.push(el);
  }) //repeating nodes in lpsl

  var rmpsl = [] 
  mpsl.forEach(function(el){
    mstate = 0;
    mvp.forEach(function(el2){if(el.key == el2.key) mstate = 1;});
    if(mstate == 1) rmpsl.push(el);
  }) //repeating nodes in mpsl
...
//  mpsl = mpsl.filter(function(element){return mvp.indexOf(element) == -1;})	//excluding visited nodes to avoid infinite regress
  ...
  while(i >= 0){
    lstate = 0;
    lvp.forEach(function(el2){if(lpsl[i].key == el2.key) lstate = 1;});
    if(lstate == 1) indices_to_be_deleted.push(i);
    i = i-1;
  } 
  for(var j=0; j<indices_to_be_deleted.length; j++){
    lpsl.splice(indices_to_be_deleted[j],1);
  } //excluding visited nodes in lpsl to avoid infinite regress

  i = mpsl.length - 1;
  indices_to_be_deleted = [];
  while(i >= 0){
    mstate = 0;
    mvp.forEach(function(el2){if(mpsl[i].key == el2.key) mstate = 1;});
    if(mstate == 1) indices_to_be_deleted.push(i);
    i = i-1;
  }
  for(var j=0; j<indices_to_be_deleted.length; j++){
    mpsl.splice(indices_to_be_deleted[j],1);
  } //excluding visited nodes in mpsl to avoid infinite regress


  matchable_nodes = lpsl.filter(function(element){
    if(element.definition != undefined) return (element.definition.slice(0,5) != 'not::' && element.definition.slice(0,5) != 'new::');
    else return true;
    }); //extracting lhs from rule graph (if lhs is a generative or interpretation rule or nac), else returning all the nodes (if lhs is a main graph)
...
//      console.log(JSON.stringify(anon_nodes));
  var current_node_type_obj = null, current_node_obj = null, current_node = null,
    ...
    if(lhs.nac_count == undefined && lhs.rule_name == undefined && lhs.name == "graph8" && mg.name == "graph7") {
      console.log("entered");
    }
...
//	  console.log(JSON.stringify(current_node.rule_index));
          current_node_obj = {'type':'node', 'name':current_node, 'child':[]};
    ...
    if(lhs.nac_count == undefined && lhs.rule_name == undefined && lhs.name == "graph8" && mg.name == "graph7") {
      console.log("lhs node key " + nlpsl[y].key);
      console.log("mg node key " + perms[x][y].key);
      console.log("lhs preds " + JSON.stringify(extract_relevant_data_from_list_of_nodes(lhs_pred)));
      console.log("mg preds " + JSON.stringify(extract_relevant_data_from_list_of_nodes(mg_pred)));
      console.log("lvp is " + JSON.stringify(extract_relevant_data_from_list_of_nodes(lvp)));
      console.log("mvp is " + JSON.stringify(extract_relevant_data_from_list_of_nodes(mvp)));
    }
	    result1 = sub_match(lhs, mg, lhs.link_types[l], lhs_pred, mg_pred, lvp, mvp);
    if(lhs.nac_count == undefined && lhs.rule_name == undefined && lhs.name == "graph8" && mg.name == "graph7") {
      console.log("came back from result1 of lhs key of " + nlpsl[y].key + " and mg key of " + perms[x][y].key + " for link type " + lhs.link_types[l]);
      console.log("lhs node key " + nlpsl[y].key);
      console.log("mg node key " + perms[x][y].key);
      console.log("lhs succs " + JSON.stringify(extract_relevant_data_from_list_of_nodes(lhs_succ)));
      console.log("mg succs " + JSON.stringify(extract_relevant_data_from_list_of_nodes(mg_succ)));
      console.log("lvp is " + JSON.stringify(extract_relevant_data_from_list_of_nodes(lvp)));
      console.log("mvp is " + JSON.stringify(extract_relevant_data_from_list_of_nodes(mvp)));
    }
	    result2 = sub_match(lhs, mg, lhs.link_types[l], lhs_succ, mg_succ, lvp, mvp);
    if(lhs.nac_count == undefined && lhs.rule_name == undefined && lhs.name == "graph8" && mg.name == "graph7") {
      console.log("came back from result2 of lhs key of " + nlpsl[y].key + " and mg key of " + perms[x][y].key + " for link type " + lhs.link_types[l]);
      console.log("lvp is " + JSON.stringify(extract_relevant_data_from_list_of_nodes(lvp)));
      console.log("mvp is " + JSON.stringify(extract_relevant_data_from_list_of_nodes(mvp)));
	    }
...
function predecessors(graph, node, link_type){
  ...
  for(var i=0; i<graph.links.length; i++){
    if((graph.links[i].definition != undefined) && ((graph.links[i].definition.slice(0,5) == 'new::') || (graph.links[i].definition.slice(0,4) == 'not:'))) continue;
...
//  console.log("entered merge node types");
  ...
  console.log(JSON.stringify(final_arr));
  return final_arr;
}

function extract_all_matches(obj){
  var final_arr = [], arr = [], temp_arr = null;
  obj.child.forEach(function(list){
    temp_arr = [[]];
    list.forEach(function(element){
      console.log(JSON.stringify(element));
      arr = extract_all_matches_recursively(element);
//      console.log("came out of extract_all_matches_recursively");
      temp_arr = cartesian_product(temp_arr, arr);
    });
    temp_arr.forEach(function(local_list){final_arr.push(local_list);});
  });
  return final_arr;
}

function extract_all_matches_recursively(obj){
  var node_type_level = [], node_level = [], link_type_level = [], obj_level = [[]], temp_arr = [[]];
//      console.log("entered extract_all_matches_recursively for obj " + JSON.stringify(obj.name));
...
//      console.log("end of extract_all_matches_recursively for object" + JSON.stringify(obj.name));
...
//  console.log("entered cartesian_product for " + JSON.stringify(a1[0]) + " " + JSON.stringify(a2[0]));
...
//  console.log("end of cartesian_product for " + JSON.stringify(a1[0]) + " " + JSON.stringify(a2[0]));
  return final_arr;
}

function extract_relevant_data_from_list_of_nodes(list){
  var final_list = [];
  list.forEach(function(el){
    final_list.push(extract_relevant_data_from_node(el));
  });
  return final_list;
}

/*************************************** NOT BEING USED RIGHT NOW *****************************************/
//if you are using this code, change this code such that element.rule_index points to the index of node in lhs, not its key.

function filter_matches_with_negative_edges(graph1, graph2, matches_list){  //almost perfect, but not handling some complex cases. Not necessary right now.
//  console.log(JSON.stringify(matches_list));
  var lhs_source_node_key = null, lhs_target_node_key = null, match_source_node_key = null, match_target_node_key = null, mg_source_node = null, mg_target_node = null, temp_successors = [], negative_edges = [];
  negative_edges = graph1.links.filter(function(element){
    if(element.definition != undefined) return (element.marker_id[0] == '!' || element.definition.slice(0,5) == 'new::');
    else return true;
  });  //return the corresponding boolean value if lhs is a generative or interpretation rule or nac; else if lhs is main graph return all nodes
//  console.log(JSON.stringify(negative_edges));
  if(negative_edges.length == 0) return matches_list;
  else{
    for(var i=0; i<negative_edges.length; i++){
//      lhs_source_node_key = negative_edges[i].source.key; 
      if(typeof negative_edges[i].source == "object") lhs_source_node_key = negative_edges[i].source.key;
      else lhs_source_node_key = graph1.nodes[negative_edges[i].source].key;
      if(typeof negative_edges[i].target == "object") lhs_target_node_key = negative_edges[i].target.key;
      else lhs_target_node_key = graph1.nodes[negative_edges[i].target].key;
      matches_list.forEach(function(element){
        match_source_node = element.filter(function(local_element){return local_element.rule_index == lhs_source_node_key;});
        match_target_node = element.filter(function(local_element){return local_element.rule_index == lhs_target_node_key;});
	if(match_source_node.length != 0 && match_target_node.length != 0){
  	  match_source_node_key = match_source_node[0].key;
  	  match_target_node_key = match_target_node[0].key;
	  mg_source_node = graph2.nodes.filter(function(local_element){return local_element.key == match_source_node_key;})[0];
	  mg_target_node = graph2.nodes.filter(function(local_element){return local_element.key == match_target_node_key;})[0];
	  temp_successors = [];
	  for(var j=0; j<graph2.links.length; j++){
	    temp_successors = temp_successors.concat(successors(graph2, mg_source_node, graph2.links[j].label));
	  }
//	  temp_successors = successors(graph2, mg_source_node, negative_edges[i].label);
//	  console.log(JSON.stringify(negative_edges[i]));
	  for(var j=0; j<temp_successors.length; j++){
	    if(mg_target_node.key == temp_successors[j].key) break;
	  }
	  if(j<temp_successors.length) {
//	    console.log("entered");
//	    console.log(JSON.stringify(matches_list));
	    matches_list.splice(matches_list.indexOf(element),1);
//	    console.log(JSON.stringify(matches_list));
	  }
	}
      });
