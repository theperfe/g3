...
/*Assumptions:
1. Both the graphs are connected graphs.
2. In the lhs, once an NAC node or chain starts, then there will be no further regular nodes to be encountered.
3. In the lhs, if we face an NAC node, then it ends there. Not checking for further NAC nodes. In other words, the NAC chain lenght is always assumed to be 1.
4. The 0th node of lhs is always a regular node, not an NAC or a new node or a node to be deleted.
5. The matching always takes the 0th node of LHS and checks it with all the nodes of main graph. The matching doesnt check for other combinations i.e. it never takes other nodes of LHS as the starting point.
*/

  var current_node_type = null;
  if(graph1.nodes.length > 0) current_node_type = graph1.nodes[0].label;   //starting with first node in lhs

  //filtering nodes with the same label from graph2. Will check only these to see if their neighbours match wide with node0 of lhs recursively or not.
  var possible_matching_nodes = graph2.nodes.filter(function(element){return element.label == current_node_type});

  //to store all the matches of graph1 (lhs) in graph2 (mg) in an array
  ...
  for(var p=0; p<possible_matching_nodes.length; p++){
   current_node = extract_relevant_data_from_node(possible_matching_nodes[p]);
   current_node.rule_index = 0;	//rule_index refers to the index of the node in the rule graph with which this node is matching.
...
function sub_match(lhs, mg, lt, lpsl, mpsl, lvp, mvp){	//lhs, main_graph, link type, lhs pred succ list, mg pred succ list, lhs visited path, mg visited path
  ...
  for(var i=0; i<(perms1.length||1); i++){
    ...
    for(var j=0; j<mg.node_types.length; j++){  //for a given node type
      ...
      for(var x=0; x<perms.length; x++){
        ...
        for(var y=0; y<perms[x].length; y++){
          current_node = extract_relevant_data_from_node(perms[x][y]);
...
function extract_all_matches_recursively(obj){
  ...
  obj.child.forEach(function(el){	//for each link type
    ...
    el.child.forEach(function(el2){ 	//for each node type
      node_type_level = [];
      el2.forEach(function(el3){	 //for each array of nodes in each node type
        temp_arr = [[]];
