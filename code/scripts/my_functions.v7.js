...
//  if(lpsl == false || mpsl == false) return false;
  console.log("entered sub match");
  if(lpsl.length == 0) return false;
...
//  var link_types_obj_arr = {'child':[]};
  ...
  for(var i=0; i<lhs.link_types.length; i++){
    var current_link_type_links = [];
    current_link_type_links = lhs.link_types.filter(function(element){});
    ...
    console.log(JSON.stringify(lvp));
	  ...
	  result2 = sub_match(lhs, mg, successors(lhs, nlpsl[y], lhs.link_types[i]), successors(mg, perms[x][y], lhs.link_types[i]), lvp, mvp);
	    console.log(result2);
	  if(result1 && result2) {
	    if(visited_lvp.indexOf(nlpsl[y] == -1)) visited_lvp.push(nlpsl[y]);
	    if(visited_mvp.indexOf(perms[x][y] == -1)) visited_mvp.push(perms[x][y]);
	    result = merge(result1, result2);
	    console.log(result);
    ...
    lvp = lvp.concat(visited_lvp);
    console.log(JSON.stringify(lvp));
  //  mvp = mvp.concat(visited_mvp);
  }
...
function predecessors(graph, node, link_type){
  var attached_links = graph.links.filter(function(element){
    if(typeof element.target == "object") return element.target.key == node.key;
    else return graph.nodes[element.target].key == node.key;
  });
  var result = [];
...
//  if (attached_links.length != 0 && result.length == 0) return false;
//  else return result;
  return result;
}
...
function successors(graph, node, link_type){
  var attached_links= graph.links.filter(function(element){
    if(typeof element.source == "object") return element.source.key == node.key;
    else return graph.nodes[element.source].key == node.key;
  });
  var result = [];
  for(var i=0; i<graph.links.length; i++){
