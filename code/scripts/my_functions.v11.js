...
function match(graph1, graph2){
  ...
  for(var p=0; p<possible_matching_nodes.length; p++){
   current_node_obj = {'type':'node', 'name':extract_relevant_data(possible_matching_nodes[p]), 'child':[]};
   console.log(current_node_obj);
  ...
  for(var i=0; i<lhs.node_types.length; i++){
    ...
    for(var x=0; x<perms.length; x++){
      ...
      for(var y=0; y<perms[x].length; y++){
        current_node_obj = {'type':'node', 'name':extract_relevant_data(perms[x][y]), 'child':[]};
	lvp.push(nlpsl[y]);
	mvp.push(perms[x][y]);
