...
function match_final(graph1, graph2){
  ...
  var lhs_source_node_key = null, lhs_target_node_key = null, match_source_node_key = null, match_target_node_key = null, mg_source_node = null, mg_target_node = null;
...
//      lhs_source_node_key = negative_edges[i].source.key; 
      ...
      final_matches.forEach(function(element){
        match_source_node_key = element.filter(function(local_element){return local_element.rule_index == lhs_source_node_key;});
        match_target_node_key = element.filter(function(local_element){return local_element.rule_index == lhs_target_node_key;});
	if(match_source_node_key.length != 0 && match_target_node_key.length != 0){
  	  match_source_node_key = match_source_node_key[0];
  	  match_target_node_key = match_target_node_key[0];
	  mg_source_node = graph2.nodes.filter(function(local_element){local_element.key == match_source_node_key;})[0];
	  mg_target_node = graph2.nodes.filter(function(local_element){local_element.key == match_target_node_key;})[0];
	  if(mg_target_node in successors(graph2, mg_source_node, negative_edges[i].label)) final_matches.splice(final_matches.indexOf(element),1);
	}
      });
    }
  }
