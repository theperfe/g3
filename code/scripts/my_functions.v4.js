...
//    graph1.link_types.forEach(function(local_element){
    for(var lt=0; lt<graph1.link_types.length; lt++){
      result1 = sub_match(graph1, graph2, predecessors(graph1, graph1.nodes[0], graph1.link_types[lt]), predecessors(graph2, element, graph1.link_types[lt]), [graph1.nodes[0]], [element])
      result2 = sub_match(graph1, graph2, successors(graph1, graph1.nodes[0], graph1.link_types[lt]), successors(graph2, element, graph1.link_types[lt]), [graph1.nodes[0]], [element])
      console.log(result1);
      if(result1 && result2) {
...
//  var result1 = null, result2 = null;
  ...
  for(var i=0; i<lhs.link_types.length; i++){
	...
	perm.forEach(function(local_element){
      ...
      for(var x=0; x<perms.length; x++){	//for each permutation
	var arr = [];
        for(var y=0; y<perms[x].length; y++){	//for each element in a given permutation
	  lvp.push(nlpsl[y]);
	  mvp.push(perms[x][y]);
	  var current_node_obj = {'type':'node', 'name':perms[x][y], 'child':[]};
	  var result1 = sub_match(lhs, mg, predecessors(lhs, nlpsl[y], lhs.link_types[i]), predecessors(mg, perms[x][y], lhs.link_types[i]), lvp, mvp);
	  var result2 = sub_match(lhs, mg, successors(lhs, nlpsl[y], lhs.link_types[i]), successors(mg, perms[x][y], lhs.link_types[i]), lvp, mvp);
	  ...
	  else break;		//if even one element in the given permutation does not match, go to the next permutation.
	}
	for(var z=y-1; z>=0; z--){
	  lvp.pop();
	  mvp.pop();
	}
	if((y == perms[x].length) && (arr.length != 0)){	//if all the elements of permutation satsfied, then push that permutation
  ...
  var new_array = $A(link_types_obj_arr);
  return new_array;
}
